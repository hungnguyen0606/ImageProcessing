...
void myCanny(Mat src, Mat& dest, int lowThreshold, int highThreshold)
{
	...
	for (int i = 0; i < n; ++i)
		...
		//	cout << i << ' ' << j << endl;
			...
			//debuging
			src.at<Vec3b>(i, j)[0] = (uint8_t)(sqrt(x*x + y*y) / sqrt(2 * 1024 * 1024)*255);
			src.at<Vec3b>(i, j)[1] = (uint8_t)(sqrt(x*x + y*y) / sqrt(2 * 1024 * 1024) * 255);
			src.at<Vec3b>(i, j)[2] = (uint8_t)(sqrt(x*x + y*y) / sqrt(2 * 1024 * 1024) * 255);
			//
